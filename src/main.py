...
def ask_ampm(default: str) -> str:
    s = input(f"  AM or PM [{default}]: ").strip().upper()
    if s == "":
        s = default
...
def main():
    print("=== CGM Simulator (Interactive) ===")
    print(f"(Fixed) Δt = {DT_MIN} minutes | Forecast horizon = {HORIZON_MIN} minutes (3 hours)")
    print(f"(Fixed) Meal t_peak = {MEAL_T_PEAK_MIN} min | Meal duration = {MEAL_DURATION_MIN} min")
    ...
    if start_glucose < 70:
        print("⚠️ Warning: Starting glucose is below 70 mg/dL (hypoglycemia range).")
    ...
    if ask_yes_no("\nAdd a meal?", True):
        ...
        if not in_window(meal_offset):
            ...
            print(f"⚠️ Meal at {fmt_time(meal_dt)} is outside the 3-hour window; ignoring it.")
        else:
            meals.append(
                Meal(
    ...
    if ask_yes_no("\nAdd insulin?", True):
        auto = ask_yes_no("Auto-calc bolus from meal carbs?", True)
        if auto and len(meals) > 0:
            ...
            if not in_window(dose_offset):
                ...
                print(f"⚠️ Insulin at {fmt_time(dose_dt)} is outside the 3-hour window; ignoring it.")
            else:
    ...
    if ask_yes_no("\nAdd exercise?", False):
        ...
        if not in_window(ex_offset):
            ...
            print(f"⚠️ Exercise at {fmt_time(ex_dt)} is outside the 3-hour window; ignoring it.")
        else:
    ...
    if ask_yes_no("\nAdd stress?", False):
        ...
        if not in_window(st_offset):
            ...
            print(f"⚠️ Stress at {fmt_time(st_dt)} is outside the 3-hour window; ignoring it.")
        else:
    ...
    if start_glucose < 70 and len(meals) == 0 and (len(doses) > 0 or exercise is not None):
        print("⚠️ WARNING: Low start glucose + insulin/exercise without carbs may cause dangerous hypoglycemia.")
